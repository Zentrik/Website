...
\begin{align*}
  ...
  \end{bmatrix} (\#eq:9)
\end{align*}
The paper imposes three inequalities in this form: constraint \@ref(eq:7); a bound on the velocity 
\begin{align*}
  || \dot{r}(t) \leq \hat{V}|| \text{ for } t \in [0, t_f] (\#eq:10)
\end{align*}
and a "glide slope" constraint
\begin{align*}
  \arctan \left( \frac{\sqrt{r_2(t)^2 + r_3(t)^2}}{r_1(t)} \right) \leq \tilde{\theta}_\text{alt} \leq \frac{\pi}{2}
\end{align*} 
where $\tilde{\theta}_\text{alt}$ is given.
...
::: {.lemma #one}
Consider a solution of [Problem 2] given by $[t_f^*, T_c^*(\bullet), \Gamma^*(\bullet)]$ <!-- such that the corresponding state trajectory satisfies $x_1^*(t) \in \operatorname{int} X \; \forall \; t \in (0, t_f^*)$^[This condition was added in @blackmore_minimum_landing_2008]. As we haven't added the constraint $r(t) \in X$, I don't think the condition is necessary -->. Then, $[t_f^*, T_c^* (\bullet)]$ is also a solution of [Problem 1] and $||T_c^*(t)|| = \rho_1$ or $||T_c^*(t)|| = \rho_2$ for $t \in [0, t_f^*]$.
:::
...
If $\lambda_1 \neq 0$ then $m \dot{R}_{12}$ is not identically 0 and as $m \neq 0$ we know $\dot{R}_{12}$ is not identically 0. When $\lambda_1 \neq 0$, we can see that $\frac{d }{d t} m \dot{R}_{12} = ||\lambda_2|| > 0$
\begin{align*}
  \ddot{R}_{12} &= \frac{||\lambda_2||}{m} - \frac{1}{m^2} \dot{m} \frac{1}{m ||\lambda_2||} (||\lambda_1||^2 t - \lambda_1^T a) \\
  &= \frac{||\lambda_2||}{m} + \frac{\alpha \Gamma}{m^2} \frac{1}{m ||\lambda_2||} (||\lambda_1||^2 t - \lambda_1^T a)
\end{align*}
Therefore it can only be zero at one point and the sign can only change from negative to positive so, there can only be two points at max where $R_{12} = 0$ so $R_{12} \neq = 0$ a.e. on [0, t_f] when $\lambda_1 \neq 0$.
