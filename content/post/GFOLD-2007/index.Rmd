...
The intial and final position and velocity, and the initial mass are specified,
\begin{align*}
...
FIX glideslope. It simply is measuring another angle as other papers. it is measuring angle from vertical to rocket, so 90 - what is in later papers.
...
Inequality \@ref(eq:3) is a nonconvex constraint on the control input and so we need to deal with it in order to obtain a convex optimisation problem.

The paper claims that $m^*(t) > 0$ in Lemma \@ref(lem:one), obviously in the real world this is true, however I don't see why this is true in [Problem 1] or [Problem 2]. In @aci2008 they add the constraint $m(t_f) \geq 0$ to their problem and in @bla2010 they add the constraint $m(t_f) \geq m_{dry}$. We will take $m(t) > 0$ to be true henceforth, however I will work out how to properly deal with this at a later date.

`The spacecraft has an available fuel amount, mfuel, that is not explicitly incorporated into the
trajectory optimization problem FOP. If the optimal fuel amount needed for FOP is larger than the available
fuel then the problem is infeasible. Another form of infeasibility occurs when there is no solution to FOP,
due to control and/or state constraints' ???

Thrust direction can also be constrained at the start and at the end of a maneuver as 
\begin{align*}
...
[Problem 2] has introduced a slack variable $\Gamma$ which replaces $|| T_c ||$ and introduces a new constraint $\left \lVert T_c(t) \right \rVert \leq \Gamma(t)$.
...
::: {.lemma #one}
Consider a solution of [Problem 2] given by $[t_f^*, T_c^*(\bullet), \Gamma^*(\bullet)]$^[such that the corresponding state trajectory satisfies $x^*(t) \in \operatorname{int} X \; \forall \; t \in (0, t_f^*)$^[This condition was added in @bla2010] ($X$ here enforces all three state constraints, so $X = \{x \in \mathbb{R}^6: ||Sx|| + c^T x \leq 0 \text{ and } || \dot{r}(t) || \leq \hat{V} \}$). I've tried to incorporate the velocity constraint idk if this right <!-- As we haven't added the constraint $r(t) \in X$, I don't think the condition is necessary. Actually we have $r_1(t) \geq 0$. -->]. Then, $[t_f^*, T_c^* (\bullet)]$ is also a solution of [Problem 1] and $||T_c^*(t)|| = \rho_1$ or $||T_c^*(t)|| = \rho_2$ for $t \in [0, t_f^*]$.
:::

::: {.proof}
Let $y = [x^T,\ m]^T$ and $x = [x_1^T, x_2^T] = [r, \dot{r}]$, then $\dot{y} = [x_2,\ g + T_c / m,\ - \alpha \Gamma]^T$. The Hamiltonian^[I'll have a post explaining the Hamiltonian and Pontryagin's principle up soon] for [Problem 2] is 
\begin{align*}
...
The set of control constraints $\Psi$ is $\left\{ (T_c, \Gamma) : ||T_c(t)|| \leq \Gamma(t),\ \rho_1 \leq \Gamma(t) \leq \rho_2 \right\}$ is a fixed set (I think this means it doesn't vary with time. Actually $\Gamma$ varies with time so I don't see how this set can't vary with time).

Let $v = [T_c^T,\ \Gamma]^T$, then there exist a $\lambda_0 \leq 0$ and an absolutely continouous function^[https://math.stackexchange.com/questions/191268/absolutely-continuous-functions seems like a nice description of what an absolutely continouous function is] $\lambda$ such that
...
2. *Pointwise maximum principle* given below must be satisified
\begin{align*}
...
3. The following *transversality condition* must be satisfied: Let $\psi = [y^{*^T}, v^{*^T}, \lambda_0, \lambda^{T} ]^{T}$, then $[ H(\psi(0)),\ -\lambda(0)^T,\ -H(\psi(t^*_f)),\ \lambda(t_f^*)^T ]^T$ must be orthorgonal to the set of feasible initial and final conditions $[0, y(0), t_f, y(t_f) ]$ at the point $[ 0,\ y^*(0)^T,\ t_f^*,\ y^*(t_f^*)^T ]$.
:::
...
To maximise the Hamiltonian per the pointwise maximum principle \@ref(eq:18) we want to maximise $R_2(t)^T T_c(t) = ||R_2(t)|| \ ||T_c|| \cos \theta$, we want to maximise $||T_c(t)|| \cos \theta$ unless $||R_2(t)|| = 0$. As $||R_2(t)|| = 0$ iff $\lambda_2(t) = 0$ as $m^*(t) \neq 0$. 
As $\lambda_2(t) \neq 0$ a.e. on $t \in [0, t_f^*]$ then we want to maximise $||T_c(t)|| \cos \theta$ a.e. on $t \in [0, t_f^*]$. 
So we want to maximise $||T_c(t)||$ as long as $\cos \theta(t) \neq 0$ (should it be $\cos \theta(t) > 0$???). 
As we can choose $\theta$ without constraint for $t \in (0, t^*_f)$^[The start/end constraints determine $\theta$], we choose $\cos \theta = 1$ for $t \in (0, t^*_f)$. 
...
\end{align*} Where has the half come from?????]
...
Additionally, I have found the following quote 'solving the resulted two-point boundary value problem is a difficult task, and good initial guesses of the adjoint variables can be difficult to find or compute' @lys2018, which backs up my experience.
