...
\begin{align*}
  ...
  &= \lambda_0 \Gamma + \lambda^{T} x_2 + \frac{\lambda^T_2 T_c}{m} + \lambda^T_2 g - \alpha \lambda_3 \Gamma (\#eq:16)
\end{align*}
with $\lambda_0 \leq 0$.
...
Now usually with Pontryagin's principle we will want to solve for the optimal control by maximising the Hamiltonian. However, this requires us to know the value of $\lambda$ and unfortunately we don't have enough boundary conditions on $\lambda$ to work it out. $\lambda_1(t^*_f)$ is not in $H(\psi(t^*_f))$ as $\dot{r}(t_f^*) = 0$, so we can't work out $\lambda_1(t^*_f)$.

Instead the paper formulates the problem as a SOCP.
