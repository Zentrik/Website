...
There are many different proofs for Pontryagin’s principle depending on if we have initial/ target sets and what assumptions we make (do we assume everything is differentiable or not?).

An example where Pontryagin’s principle has been applied is the [pin-point landing problem](/post/gfold-2007).

# Proofs

I will outline the different proofs below.
...
We want a variation that causes a small change to the state as we don't want to violate any state constraints.
...
I haven't been able to fully work out when $\lambda = 0$ (it may be an open problem) so below I list some results concerning $\lambda_0$.
