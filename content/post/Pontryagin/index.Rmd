...
Pontryagin’s principle is a set of neccesary conditions for an optimal solution to a dynamical system.

The general problem is stated as 
\begin{align*}
  \min K(t_0, x(t_0), t_f, x(t_f)) + \int_{t_0}^{t_f} L(x, u, t) \\
  s.t. \dot{x} = f(x, u, t),\ (t_0, x(t_0), t_f, x(t_f)) \in \mathcal{B}
\end{align*}
$u \in U$ constraint?

This in the form of a bolza problem @bolza and can be converted to a Mayer problem, where $L \equiv 0$, or a Lagrange problem, where $K \equiv 0$ quite easily.

The time dependance in the dynamics and the cost can also be easily incorporated into the state by augmenting the state with a variable representing time.

This problem is stated as, where $x$ is the augmented state and $L$ the agumented cost
\begin{align*}
  \min &\int_{t_0}^{t_f} L(x, u) \\
  s.t. \dot{x} = f(x, u)&,\ (x(t_0), x(t_f)) \in \mathcal{B}
\end{align*} 

Pontryagin’s principle for this problem is as follows.

(i) There exist an absolutely continouous function $\hat{\lambda} = (\lambda_0, \lambda)$ defined on $[t_0, t_f]$ such that $\lambda_0$ is either identically minus one or zero, and $\hat{\lambda} \neq \boldsymbol 0$ for all $t \in [t_0, t_f]$

(ii)
\begin{align*}
  \dot{\lambda} = -\frac{\partial H}{\partial x} \left( x^*,\ u^*,\ \lambda_0,\ \lambda \right)
\end{align*} 

iii
\begin{align*}
  H(x^*(t),\ u^*(t),\ \hat{\lambda}(t)) \geq H(x^*(t),\ u,\ \hat{\lambda}(t)) \hspace{0.3cm} \; \forall \; u \in \Psi \hspace{0.3cm} \text{a.e. on } t \in [t_0, t^*_f]
\end{align*} 
Simply put the Hamiltonian must be maximised with respect to $u$ at all times.

(iiii
The following *transversality condition* must be satisfied: Let $\psi = [x^{*^T}, u^{*^T}, \hat{\lambda}^{T} ]^{T}$, then $[ H(\psi(t^*_0)),\ -\lambda(t^*_0)^T,\ -H(\psi(t^*_f)),\ \lambda(t^*_f)^T ]^T$ must be orthorgonal to $\mathcal{B}$ at the point $[ t^*_0,\ x^*(t^*_0)^T,\ t^*_f,\ x^*(t^*_f)^T ]$.

Cite and fix superscript stars
...
We can use a mcluarin series to see that the following is true:
\begin{align*}
  x_\epsilon(t) &= x^*(t) + \epsilon \left. \frac{\partial}{\partial \epsilon} \right|_{\epsilon=0^+} x_\epsilon(t) + o(\epsilon),\ \text{for $t \geq \tau$} \\
  &= x^*(t) + \epsilon v(t) + o(\epsilon),\ \text{for $t \geq \tau$}
\end{align*} 
Let $\tau = 0$ and so \begin{align*}
  \dot{x}_\epsilon(t) &= \dot{x}^*(t) + \epsilon \dot{v}(t) \\
  \dot{x}_\epsilon(t) &= f(x_\epsilon(t), u^*(t)) \\
...
\begin{align*}
  ...
  \end{cases} 
\end{align*} 
where $\tau \in (0, T]$ and $0 < \epsilon < \tau$ and $\omega \in U$.
...
Now to finish the proof we can note that $v(\tau) = f(x^*(\tau), \omega) - f(x^*(\tau), u^*(\tau))$ a.e. on $\tau \in [0, T]$. So setting $\tau = 0$ completes the proof.

We can note that $v(t) = \left. \frac{\partial}{\partial \epsilon} \right|_{\epsilon=0^+} x_\epsilon(t)$ where $x_\epsilon$ is simply a variation in the inital condition by $\epsilon v(t_0)$. @aro23
@aro123
...
These are old notes, they are unpolisihed and will barely make sense. 
